package androidx.car.app;

import android.content.Intent;
import android.os.Binder;
import android.os.IBinder;
import android.os.IInterface;
import android.os.Parcel;
import android.os.Parcelable.Creator;

public interface IStartCarApp extends IInterface {
    public static class Default implements IStartCarApp {
        @Override  // android.os.IInterface
        public IBinder asBinder() {
            return null;
        }

        @Override  // androidx.car.app.IStartCarApp
        public void startCarApp(Intent intent0) {
        }
    }

    public static abstract class Stub extends Binder implements IStartCarApp {
        static class Proxy implements IStartCarApp {
            private IBinder mRemote;

            public Proxy(IBinder iBinder0) {
                this.mRemote = iBinder0;
            }

            @Override  // android.os.IInterface
            public IBinder asBinder() {
                return this.mRemote;
            }

            public String getInterfaceDescriptor() {
                return "androidx.car.app.IStartCarApp";
            }

            @Override  // androidx.car.app.IStartCarApp
            public void startCarApp(Intent intent0) {
                Parcel parcel0 = Parcel.obtain();
                Parcel parcel1 = Parcel.obtain();
                try {
                    parcel0.writeInterfaceToken("androidx.car.app.IStartCarApp");
                    if(intent0 == null) {
                        parcel0.writeInt(0);
                    }
                    else {
                        parcel0.writeInt(1);
                        intent0.writeToParcel(parcel0, 0);
                    }
                    this.mRemote.transact(2, parcel0, parcel1, 0);
                    parcel1.readException();
                }
                finally {
                    parcel1.recycle();
                    parcel0.recycle();
                }
            }
        }

        static final int TRANSACTION_startCarApp = 2;

        public Stub() {
            this.attachInterface(this, "androidx.car.app.IStartCarApp");
        }

        @Override  // android.os.IInterface
        public IBinder asBinder() {
            return this;
        }

        public static IStartCarApp asInterface(IBinder iBinder0) {
            if(iBinder0 == null) {
                return null;
            }
            IInterface iInterface0 = iBinder0.queryLocalInterface("androidx.car.app.IStartCarApp");
            return iInterface0 != null && iInterface0 instanceof IStartCarApp ? ((IStartCarApp)iInterface0) : new Proxy(iBinder0);
        }

        @Override  // android.os.Binder
        public boolean onTransact(int v, Parcel parcel0, Parcel parcel1, int v1) {
            if(v >= 1 && v <= 0xFFFFFF) {
                parcel0.enforceInterface("androidx.car.app.IStartCarApp");
            }
            switch(v) {
                case 2: {
                    Parcelable.Creator parcelable$Creator0 = Intent.CREATOR;
                    this.startCarApp(((Intent)(parcel0.readInt() == 0 ? null : parcelable$Creator0.createFromParcel(parcel0))));
                    parcel1.writeNoException();
                    return true;
                }
                case 0x5F4E5446: {
                    parcel1.writeString("androidx.car.app.IStartCarApp");
                    return true;
                }
                default: {
                    return super.onTransact(v, parcel0, parcel1, v1);
                }
            }
        }
    }

    public static final String DESCRIPTOR;

    static {
        IStartCarApp.DESCRIPTOR = "androidx.car.app.IStartCarApp";
    }

    void startCarApp(Intent arg1);
}

