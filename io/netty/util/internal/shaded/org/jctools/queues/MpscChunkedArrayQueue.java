...
public class MpscChunkedArrayQueue extends MpscChunkedArrayQueueColdProducerFields {
    ...
    public MpscChunkedArrayQueue(int v) {
        super(Math.max(2, Math.min(0x400, Pow2.roundToPowerOfTwo(v / 8))), v);
    }

    public MpscChunkedArrayQueue(int v, int v1) {
        super(v, v1);
    }
    ...
    public long availableInQueue(long v, long v1) {
        return this.maxQueueCapacity - (v - v1);
    }
    ...
    public int fill(Supplier messagePassingQueue$Supplier0) {
        return super.fill(messagePassingQueue$Supplier0);
    }
    ...
    @Override  // io.netty.util.internal.shaded.org.jctools.queues.BaseMpscLinkedArrayQueue
    public int getNextBufferSize(Object[] arr_object) {
        return LinkedArrayQueueUtil.length(arr_object);
    }
