...
public enum SDKMessageEnum {
    ...
    private SDKMessageEnum(String s1) {
        this.mStringValue = s1;
    }

    public static SDKMessageEnum fromString(String s) {
        SDKMessageEnum[] arr_sDKMessageEnum = SDKMessageEnum.values();
        for(int v = 0; v < arr_sDKMessageEnum.length; ++v) {
            SDKMessageEnum sDKMessageEnum0 = arr_sDKMessageEnum[v];
            if(sDKMessageEnum0.toString().equals(s)) {
                return sDKMessageEnum0;
            }
        }
        return null;
    }
